...
import mx.uacm.hibernateapp.dominio.ClienteDTO;
...
public class HibernateQL {
    public static void main(String[] args) {
        ...
        Object[] objetoCliente = em.createQuery("select c.id, c.nombre, c.apellido from Cliente c where c.id=:id", Object[].class) //varios campos tipo generico
                .setParameter("id", 1L)
                .getSingleResult();
        ...
        List<ClienteDTO> clientesDto = em.createQuery("select new mx.uacm.hibernateapp.dominio.ClienteDTO(c.nombre, c.apellido) from Cliente c", ClienteDTO.class)
                .getResultList();
...
//        clientes = em.createQuery("select c from Cliente c where c.id between 2 and 5", Cliente.class).getResultList();
        ...
        /*System.out.println("========== consulta con el nombre mas Largo y mostrar su nombre ==========");
        Object[] maxLargoNombre2 = em.createQuery("select max(length(c.nombre)) as Longitud, c.nombre from Cliente c group by(c.nombre) limit 1", Object[].class).getSingleResult();
        System.out.println(maxLargoNombre2);*/
