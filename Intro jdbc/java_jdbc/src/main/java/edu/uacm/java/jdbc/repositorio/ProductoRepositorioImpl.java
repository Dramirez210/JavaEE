package edu.uacm.java.jdbc.repositorio;
...
public class ProductoRepositorioImpl implements Repositorio<Producto> {
    ...
    public List<Producto> listar() {
        ...
        try (Statement stmt = getConnection().createStatement();
             ResultSet rs = stmt.executeQuery("SELECT p.*, c.nombre as Categoria FROM productos p " +
                     ...
                     "p.categoria_id = c.idcategorias")) { // autoclose
    ...
    public Producto porId(Long id) {
        ...
        try (PreparedStatement stmt = getConnection().
                prepareStatement("SELECT p.nombre, p.precio, c.nombre as categoria FROM productos as p INNER JOIN categorias as c ON (p.categoria_id = c.idcategorias) WHERE idproductos = ?")) {
            stmt.setLong(1, id);
    ...
    public void guardar(Producto producto) {
        ...
        try (PreparedStatement stmt = getConnection().prepareStatement(sql)) {
            ...
            if (producto.getId() != null && producto.getId() > 0) {
                stmt.setLong(4, producto.getId()); // 3er parametro de la consulta
            } else {
                stmt.setDate(4, new Date(producto.getFechaRegistro().getTime())); // Milisegundos
            }
